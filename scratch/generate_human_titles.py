...
def clean_food_title(f):
    ...
    if 'rice' in p0_lower or ('cereal' in p0_lower and 'rice' in full_lower):
        ...
        return f"{color} Rice ({grain}{state})".replace(' ()', '').replace('( )', '').strip() if state or grain else f"{color} Rice"
    ...
    if cat == 'Vegetables and Vegetable Products':
        ...
        if 'carrot' in veg.lower(): return f"Carrots{mod_str}"
    ...
    if cat == 'Fruits and Fruit Juices':
        ...
        if 'apple' in fruit.lower(): return f"Apple{state_str}"
    ...
    if cat in ['Beef Products', 'Poultry Products', 'Pork Products', 'Lamb, Veal, and Game Products', 'Finfish and Shellfish Products']:
        ...
        if 'salmon' in desc:
            ...
            variety = 'Atlantic ' if 'atlantic' in desc else ('Sockeye ' if 'sockeye' in desc else ('Pink ' if 'pink' in desc else 'Wild '))
...
# Generate and inspect all 1804 titles
new_titles = []
for f in foods:
    new_t = clean_food_title(f)
    new_titles.append((f.get('food_id'), f.get('name'), f.get('title'), new_t, f.get('category')))

with open('scratch/new_titles_audit.txt', 'w', encoding='utf-8') as out:
    for fid, name, old_t, new_t, cat in new_titles:
        out.write(f"ID: {fid}\nNAME: {name}\nOLD: {old_t}\nNEW: {new_t}\nCAT: {cat}\n---\n")

print(f"Generated {len(new_titles)} human-friendly titles in scratch/new_titles_audit.txt")
