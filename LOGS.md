...
- **2026-09-15 04:45 PM** - Built `StepOnePage` (`lib/pages/onboarding/step_one_page.dart` v1.2.0) strictly from Figma node `387:9071`: pure unselected state (no prefilled values), `TextInput` with placeholder "Name", `TextInput` with placeholder "Date of birth (DD/MM/YYYY)" and `_DateInputFormatter`, `ToggleText` row with unselected "Male" (`Icons.face_5_outlined`) and "Female" (`Icons.face_3_outlined`).
- **2026-09-15 04:50 PM** - Built `OnboardingPage` coordinator (`lib/pages/onboarding/onboarding_page.dart` v1.5.0): mapped and persisted Step 1 user inputs (`name`, `dateOfBirth`, `sex`) directly to `UserProfile` in Isar on Next action.
- **2026-09-15 04:55 PM** - Upgraded `HorizonApplicationHeader` (`lib/horizon/horizon_application_header.dart` v1.4.0): overrode hardcoded white fill to `AlterSemanticTokens.baseGray` (`#F9FAFB`) matching Figma transparent header spec, added direct title/subtitle support for Onboarding, and handled edge-to-edge top status-bar padding.
- **2026-09-15 05:00 PM** - Updated `UserProfile` model (`lib/data/models/user_profile.dart`) to have unpopulated defaults (`name = ''`, `dateOfBirth = null`, `sex = null`, `int? get age`, `weightKg = null`, `heightCm = null`), updated `DemographicLookup` with null-safe adult fallback, and regenerated Isar schema with `build_runner`.
- **2026-09-15 05:05 PM** - Built `StepTwoPage` (`lib/pages/onboarding/step_two_page.dart` v1.2.0): `NumericInput` for Weight (`20-300kg`) and Height (`50-250cm`), and Goal selector from Figma node `400:10530` ("What is your current goal?", `Bulk 💪`, `Cut 📉`, `Maintain`) mapped to `UserProfile.goal`.
- **2026-09-15 05:10 PM** - Upgraded `OnboardingPage` coordinator (`lib/pages/onboarding/onboarding_page.dart` v1.9.0): integrated Step 2 with weight, height, and goal selection, persisting to `UserProfile` in Isar.
- **2026-09-15 05:15 PM** - Upgraded `StepOnePage` (`lib/pages/onboarding/step_one_page.dart` v1.3.0): added `32` character limit on Name, enforced DOB $\ge 12$ years validation with live error display, and added focus dismissal (`unfocus()`) on sex selector tap.
- **2026-09-15 05:20 PM** - Upgraded `StepTwoPage` (`lib/pages/onboarding/step_two_page.dart` v1.3.0): explicitly set `hasIcon: false` on the Goal Selector `ToggleText` options (`Bulk 💪`, `Cut 📉`, `Maintain`).
- **2026-09-15 05:30 PM** - Built `StepThreePage` (`lib/pages/onboarding/step_three_page.dart` v1.0.0) from Figma node `387:10075`: scrollable `Wrap` layout with multi-select `ToggleText`s (`hasIcon: false`) for all `isVisibleOnApp` nutrients, unselected by default, Next gating on $\ge 1$ selection, and integrated into `OnboardingPage` (v1.10.0) persisting to `UserProfile.nutrientTargets` in Isar.
- **2026-09-15 05:35 PM** - Finalized Onboarding to 3-step flow (`OnboardingPage` v1.11.0): removed Step 4, set Step 3 CTA label to 'Launch Horizon', and synchronized selected `UserProfile.nutrientTargets` to set matching `NutrientInfo.isTracked = true` (and all others `false`) in Isar.
