...
/// Reusable Add to the Day Food Source modal component for Horizon.
///
/// Features:
/// - Vertical layout, 24px padding, 16px gap, rounded corners 24px, base-white background.
/// - Covers the Track Page area precisely with transparent barrier (no dulled background).
/// - Child 1: `HorizonTitleBar` ('Add to the day' / 'Top-up nutrients with a new source').
/// - Child 2: `FoodSourcesListContainer` (gap=16, no padding) with `HorizonListItem`'s ADD variant.
///   - Alphabetically sorted food sources (all untracked foods, minus ones in routine).
///   - Reactively filterable by Application Header's `NutrientMap` (`selectedNutrientKey`).
///   - `ButtonIconGhost` of `add_circle` immediately adds the selected food source to today's consumption.
///   - Modal automatically closes on addition (only one source added at a time).
...
class HorizonAddSourceTrack extends StatefulWidget {
  ...
  /// Opens HorizonAddSourceTrack as a bottom-up modal covering the Track page area.
  static Future<void> show(
    ...
    return showModalBottomSheet(
      ...
      builder: (context) => HorizonAddSourceTrack(
  ...
  @override
  State<HorizonAddSourceTrack> createState() => _HorizonAddSourceTrackState();
}

class _HorizonAddSourceTrackState extends State<HorizonAddSourceTrack> {
  ...
  Future<void> _handleAddFoodToDay(FoodSourceItem food) async {
    await _trackingService.addFoodToDay(foodId: food.foodId);
    if (!mounted) return;
    widget.onDone?.call();
  }
          ...
          : Column(
              ...
              children: [
                ...
                const HorizonTitleBar(
                  ...
                  subtitle: 'Top-up nutrients with a new source',
                ),
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return HorizonListItem(
          ...
          onRightActionTap: () => _handleAddFoodToDay(food),
        );
      },
    );
  }
